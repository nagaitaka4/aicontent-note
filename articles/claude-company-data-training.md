...
この記事では、そのうち「入力した内容がモデル学習に使われるか」に絞って確認します。
情報漏洩やセキュリティ全般は、別の話です。
...
【メモ】<br>
ローカルのClaude Codeでは、PC側にもセッション記録が残る。<br>
~/.claude/projects/ に平文で標準30日。cleanupPeriodDays で変更可
...
なお、この記事で扱うのは「学習に使われるか」だけです。
情報が外に漏れるかどうかは別の話で、==オフにしたから安全、とは言えません==。
...
Claude Code公式は、Free・Pro・Maxを「consumer users」、Team・Enterprise・APIを「commercial users」として分けています。
Pro・Maxではモデル改善を設定で選び、Teamでは既定でコンテンツをモデル学習に使いません。
...
3. 自分以外の情報を日常的に入れるなら、モデル改善をオフにするか、Teamなどの商用プランを検討するか、そもそも渡さないかを決める
