...
社員は既定では会話を読めず、読めるのは使用ポリシーの確認などに限られます（[プライバシーセンター](https://privacy.claude.com/en/articles/10458704-how-does-anthropic-protect-the-personal-data-of-claude-users)）。

つまり、渡した情報がすぐ誰かの目に触れるわけではありません。
残るのは、次の4つです。

・設定がオンなら、入力は将来の学習に入る。あとから取り消せない<br>
・オフでも、会話は自分のアカウントに残り続ける<br>
・共有リンクを作った会話は、リンクを知る人なら誰でも読める（[公式ヘルプ](https://support.claude.com/en/articles/16762437-public-links-for-shared-chats)）<br>
・Claude Codeの記録はPCに平文で残る。共用のPCなら、そこからも読める

【アラート】<br>
危ないのは情報そのものより、==アカウントやPCに入られたとき==に、<br>
残っている文字列がそのまま使えてしまうこと
