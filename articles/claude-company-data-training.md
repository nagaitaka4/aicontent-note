...
そのうえで、消すことと、情報自体を使えなくすることに分けて動きます。
...
・Claude Codeの記録はPCに平文で残る。共用のPCなら、そこからも読める<br>
・安全性の検出に引っかかった会話は、オフでも安全性の改善に使われる場合がある

危ないのは、パスワードやカード番号のように、==それ自体が使える情報==です。
どこに1つ残っていても使われてしまうので、消すより先に、使えなくします。

一方、取引先名や売上のような情報は、使えなくすることができません。
こちらは、どこに残っているかを減らす方向で考えます。

| 渡してしまったもの | まずやること |
|---|---|
| パスワード・ログイン情報 | そのパスワードを変える |
| APIキー・トークン | 発行元で無効にして、作り直す |
| クレジットカード・口座の情報 | カード会社や金融機関に連絡して、番号を変える |
| 取引先や顧客の情報 | その会話を削除する。社内に報告の決まりがあれば、それに従う |

共通しているのは、「消す」だけで終わらせないことです。
渡した文字列そのものを使えなくすれば、あとで見られても困りません。

【グッド】<br>
会話を削除すると履歴からはすぐ消える。<br>
バックエンドからは30日以内に消え、今後の学習にも使われない

ただし、すでに始まっている学習からは取り消せません。
だから、パスワードやキーは変えるところまでやります。
