...
# 意味は通るが人が声に出して言わない形。既存の公開記事でヒット0本のものだけを入れている。
NG_AI_PHRASES = [
...
# 2026-08-19：「しましょう」だけでは「進めていきましょう」「見ていきましょう」が漏れる
NG_INSTRUCTION = ["ましょう"]
...
def main(path):
    ...
    print("\n--- 一文の長さチェック（60字上限） ---")
    # 2026-09-03：`![alt](url)` の画像行は読者が読む文ではないので、先に落とす。
    # （リンクの `[text](url)` はテキストを残すが、画像はaltごと対象外にする）
    sent_src = strip_decoration(
        re.sub(
            r"\[([^\]]*)\]\([^)]*\)",
            r"\1",
            re.sub(r"^!\[[^\]]*\]\([^)]*\)$", "", body_for_para_check, flags=re.M),
        )
    )
    long_sents = []
    all_sents = []
    ...
    # 2026-08-13：このチェックは文字列の類似度しか見ておらず、言い換えられた同義文は素通りする。
    # no.57でリード1文目と1章1文目が同じ定義を別の言い回しで書いていたが検出できなかった。
    print("   ⚠️ このチェックは言い換えを検出できない。下の目視チェックで意味の重複を必ず確認する")
    ...
    for item in [
        ...
        "【意味の重複】同じ内容を別の言い回しで2回書いていないか（類似度チェックは言い換えを検出できない）",
        "  └ 特に見る4箇所：①リード↔1章の定義 ②【ポイント】ブロック↔同じセクションの地の文",
        "  └ ③まとめ↔本文の再掲 ④H2見出し↔その直下のリード文（見出しの言い直しになっていないか）",
    ]:
        print(f"  - {item}")
