...
| `PERM-01`（**2026-09-10 07:32に投稿済み**・9/14午前に24ビュー・X編集部の記録。9/14訂正） | 投稿済み | 「許可80件・確認6件・拒否0件」「8/29の更新でBashの一部が自動から確認に」「9/2の更新では作業フォルダの外を読むときも確認」「確認を減らすための設定のはずが、1週間で2回増えてます」「Macをまだ更新してないので、気づいてませんでした」 |
...
**記事とXの分担（2026-09-14訂正）**：`PERM-01`は9/10に投稿済み。記事が出たら`AUTOTAB-01`か新しいリンク型で出す（`rules/x-post-flow.md`「記事を公開したら、その週にリンク型を1本入れる」）。判断はX編集部。

**⚠️ 2026-09-14（X本部）：`PERM-01`は9/10に投稿済みだった（記録漏れ）。**上の分担は成り立たないので、**記事公開時のリンク型は`PERM-01`の組み替えではなく別の角度で組む**（例：`AUTOTAB-01`の411件）。**記事本文で「許可80件・確認6件・拒否0件」を使うのは問題ない**（Xで先に出たのは数字だけで、記事の芯「消えない確認の正体」は出ていない）
