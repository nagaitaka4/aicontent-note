...
**本日の最重要は、8/21に「展開中」と記録したAugust 2026 spam updateが完了していたこと。所要2日16時間。**9/1のGSC月次スナップショットの読み方が変わる（下記）。
**あわせて、8/21に「本文未確認・次回持ち越し」とした2件（AI Modeボタンのテスト／パーソナライズ機能）を本日消化した。**

### 検索トレンド・アルゴリズム変化

- **【最重要・8/21の宿題を確定】August 2026 spam update は完了している。所要 2 days, 16 hours**（**一次情報を自分で開いて確認**：[Google Search Status Dashboard・ranking updates 履歴](https://status.search.google.com/products/rGHU1u87FJnkP6W2GwMi/history)。開始`18 Aug 2026`・**duration欄が埋まり、完了扱いの履歴に入っている**）
  - 8/21のフル版の記録（`duration欄が空で"Information"表示のまま`）から進んだ。**開始8/18＋2日16時間＝8/20〜8/21のうちに終わっている**
  - 比較（同ダッシュボードの2026年）：3/24 spam ／ 3/27 core（12日4時間）／ **21 May 2026 core（11日21時間）** ／ 6/24 spam（2日1時間）／ 8/18 spam（**2日16時間**）。**スパムアップデートは2日前後、コアアップデートは11〜12日**という差がはっきりしている
  - **9/1のGSC月次スナップショットへの反映（近々一時タスク#18）**：8/21に書いた注記は「**展開期間と重なる可能性を織り込んで読む**」だったが、**展開は8月中に完了している。**したがって9/1に見る数字は「展開中で不安定」ではなく「**展開後の状態が10日分たまったもの**」。⚠️ ただし**8月の月次データには、更新前（8/1〜8/17）と更新後（8/21〜8/31）が混ざる**。月次の平均順位を前月と比べても、どちらの影響か切り分けられない。**日別で8/18前後の段差を見るのが正しい読み方**
  - ⚠️ **これはコアアップデートではない**（8/21の記録どおり）。新しいスパムポリシーの発表は伴わない執行で、**サイト側にやることは増えていない**

- **【8/21の宿題を消化】「Google検索」ボタンをAI Modeの3ボタンに置き換えるテスト**（[suzukikenichi.com 2026-08-20](https://www.suzukikenichi.com/blog/is-the-google-search-button-disappearing-google-tests-ai-mode-replacements/)・**本日本文を開いて確認**）
  - 内容：`Google 検索`ボタンが**`Create images` / `Ask about files` / `Brainstorm`の3つ**に置き換わる
  - **対象は米国・英語検索・デスクトップのみ。日本語検索では確認されていない**
  - **Google公式コメントあり**（検索プロダクト部門VP Robby Stein・2026-08-10のポスト）：`This is a small test on desktop to help people find new things they can do with Search.`。**検索ボックスの使い方は変わらず、質問を入力してEnterを押す従来の動線は維持される**と明言
  - ⚠️ **X投稿にも記事にも採らない。**理由は3つ：①**テストであって変更ではない** ②**日本語検索は対象外**で読者の手元では何も起きない ③Google自身が「小さなテスト」「コア機能に影響しない」と言っている。**「Google検索ボタンが消える」と書いたら煽りになる**
  - **記録として残す価値**：8/21に「テストの規模・対象国・出典が未確認のため使わない」と保留した判断が、本文を開いて**正しかったと確定した**（規模＝small test・対象＝米国デスクトップ）。**見出しだけで書いていたら誤った記事になっていた**

- **【8/21の宿題を消化】Googleが検索・Discover・Googleニュースにパーソナライズ機能を導入**（[suzukikenichi.com 2026-08-21](https://www.suzukikenichi.com/blog/google-introduces-new-personalization-features-for-search-discover-and-google-news/)・一次情報は[Google公式ブログ](https://blog.google/products-and-platforms/products/search/personalize-search-discover-news/)）
  - 3機能：①**優先ソースボタン**（お気に入りのパブリッシャーを優先表示できる）②**Discoverのおすすめを自然言語で調整**③**音声ブリーフィングのカスタマイズ**（Android）
  - **サイト運営者への影響は方向が定まっていない**（記事の記述）：`パーソナライズ機能がフィードでの露出を増やすかもしれない一方で、減らすかもしれません`
  - ⚠️ **X投稿には採らない**：影響の方向が「増えるかも／減るかも」で、**読者が何をすればいいかを書けない**（軸②の採用基準「実務に落として語れる」を満たさない）
  - **ただし①の優先ソースボタンは中期の論点になる**：読者が**特定の発信元を指名して優先表示できる**なら、指名される側になれるかどうかが分かれ目になる。**GEOの「引用されるか」とは別軸で、「思い出してもらえるか」の話。**次回以降、日本での提供状況を追う

- **GoogleのJSON-LD解析が変更、HTMLアンエスケープは1回だけに**（[suzukikenichi.com 2026-08-24](https://www.suzukikenichi.com/blog/google-changes-json-ld-parsing-to-a-single-html-unescaping-pass/)）。⚠️ **本文未確認。**構造化データを手書きしていないこのサイト（SWELL＋プラグイン任せ）では対応が要るとは考えにくいが、**次回本文を開いて「WordPressテーマ任せでも影響があるか」だけ確認する**

### GEO・AI検索の動向

- **【⚠️裏取り未完・要注意】「コンテンツ品質は文字数・制作時間では測れない、Googleが見るのは『ページに残る仕事の証拠』」**（[suzukikenichi.com・SEO Weekly Update 2026-08-24](https://www.suzukikenichi.com/blog/seo-weekly-update-august-2026-week3/)）
  - **本日本文を開いたが、記事本体に一次情報が書かれていない。**根拠（Googleの誰の発言か・公式ブログか動画か）が本文になく、**解説はミエルカチャンネルの動画側にある**構成だった
  - 周辺検索で確認できたのは、**Googleの品質評価ガイドライン（QRG）が定義する`Effort`**（`Consider the extent to which a human being actively worked to create satisfying content.`）と、2024年のGoogle API文書流出で知られた`contentEffort`という項目。**どちらも2026年8月の新事実ではない**
  - ⚠️ **この項目は現時点では使えない。**理由は`M-12`とまったく同じ（**数値・根拠の所在が動画側にあり、一次情報を特定できていない**）。**8/17の教訓「要約は数字だけでなく"出典の所在"も作り出す」がそのまま当てはまる**
  - **ただしテーマ自体はこのメディアの中心に刺さる**：このリポジトリは**一文70字・総量4,000字上限**という機械チェックを運用しており（2026-08-05〜）、**「文字数では測れない」はその運用の前提そのものへの問い**になる。**記事ネタ`M-14`としてバックログに置き、動画の元ネタ（Googleの一次発言）を特定できたときに着手する**

- **Search Consoleの生成AIパフォーマンスレポート／AI Overviewオプトアウト：状況変化なし**（保留中記事`articles/search-console-ai-overview-optout.md`＝no.52の再開判定・**省略禁止項目**）
  - 本日の検索で確認できた範囲：レポートの提供開始は**2026-06-03**、見られるのは**表示回数・ページ・国・デバイス・日付の5項目のみ**（クリック・CTR・クエリは含まれない）、データは**2026-05-18以降**で遡及なし。**オプトアウトのスイッチはGoogleが「テスト中」と説明している段階**——**すべて8/14・8/17の記録から変わっていない**
  - ⚠️ **「2026年7月には日本のサイトでも順次確認できるようになっている」と書く日本語解説記事があるが、このプロパティでは7/28・8/03・8/14の3回すべて未展開だった。****判断ルールは維持する：報道を根拠に着手せず、実機で見えた日を起点にする**
  - **次の確認は9/1のGSC月次スナップショット時**（近々一時タスク#18に統合済み）。**単独で見に行かない**
...
- **【今回最大の収穫・保留記事の再開条件に到達】Search Consoleの生成AIパフォーマンスレポートが、実質すべてのサイトで見られる状態になった（2026-08-11〜12に観測）**
  ...
  - **`operations/article-backlog.md`の保留記事（no.52ドラフト・`articles/search-console-ai-overview-optout.md`）の再開判定**：2026-07-28に「日本のSearch Consoleに実装され次第、実際に使ってみた実況記事として書き直す」という条件で保留していたため、実機で確認した
  - **【同日12:23・実機確認の結果 → aicontent-note.comには未展開。記事は保留継続】**CCがアプリ内ブラウザでaicontent-note.comのSearch Console（プロパティ`https://aicontent-note.com/`）を直接確認した。**4か所すべてで生成AI関連の項目は存在しなかった**
    1. **左ナビ**：サマリー／分析情報／**検索パフォーマンス（単独リンク）**／URL検査／インデックス作成／エクスペリエンス／拡張／セキュリティと手動による対策／リンク／実績／設定。**「検索パフォーマンス」が検索結果・Discover・生成AIに分岐していない**（全開放されたプロパティでは分岐する）
    2. **検索タイプのフィルタ**：**ウェブ／画像／動画／ニュースの4つのみ**。生成AI・AI機能の選択肢なし
    3. **設定ページ**：全般設定（所有権の確認・ユーザーと権限・協力者・アドレス変更・一括データエクスポート）／クロール（robots.txt・クロールの統計情報）／概要（プロパティ追加日・削除）のみ。**AIのオプトアウトのトグルなし**
    4. **「検索での見え方」タブ**：行が1件もない空のテーブル。AI Overview等の見え方タイプも出ていない
    - なお「**AI を使用してパフォーマンス レポートをカスタマイズする**」ボタンは存在するが、これは**2026-08-03に確認済みのレポート作成アシスタント**であって生成AIパフォーマンスレポートとは別物（8/3の判定どおり）
  - **これで実機確認は7/28・8/3・8/14の3回連続で未展開。**「グローバル展開が完了した」という報道と、このプロパティに実際に降りてくるタイミングには、**2週間以上のラグが繰り返し発生している**。**今後は報道を根拠に着手判断をしない。実機で見えた日を起点にする**
  - **次の確認タイミング**：9/1のGSC月次スナップショット取得時にまとめて見る（単独で頻繁に見に行く価値はない）。tasks/README.mdの近々一時タスク#21に記録済み
