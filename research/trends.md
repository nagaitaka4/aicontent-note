...
- **【追記・9/24 07時台】ユーザーに届いたメール「New in Claude Code: Opus 5.5 and a limit reset」（Claude Team・9/24 05:32受信・ユーザーのスクリーンショット）**＝9/22のOpus 5.5と「無料でリセット」の案内で、**クラウドセッションのクレジットとは別物**。本文とfine printのverbatim：
  - `You have one limit reset* to use any time before October 22 at 9 am PT. It refills your 5-hour and weekly limits. To use your limit reset, go to Settings → Usage on the web or in Claude Desktop.`
  - `5-hour limits are 20% higher on every model. Weekly limits aren't changing.`
  - 表「Which model when」：Sonnet 5＝`Small edits and quick lookups`／`Included in your plan`、Opus 5.5（Your default）＝`Features, debugging and code review`／`Included in your plan`、**Fable 5.1＝`The longest, hardest runs`／`Up to half your weekly limit`**
  - fine print：`*Limit reset: available to individual Claude Pro and Max subscriptions created before September 22, 2026, 9:00 AM PT. Free trials excluded. One single-use reset per eligible account; must be used before October 22, 2026, 9:00 AM PT. Forfeited if you cancel or downgrade before using it. No cash value. Not transferable.`
  - **使い道**：`research-20260923-01`の条件（9/22以前の個人Pro/Max・無料トライアル除外・解約/ダウングレードで失効）と、no.27（課金）のFableの消費（週次上限の半分まで）の一次情報
...
**①②とも差分なし。**巡回6面すべてを開いて、前回（9/20）以降の新規は0件だった。**キュー投入0本・`ideas.md`への追記0件。**

### Claude Code / Anthropic

- **【差分なし】npm `@anthropic-ai/claude-code`**（本日CCがcurlで取得）：最新は**`2.1.278`**（公開`2026-09-19T01:48:59.758Z`＝日本時間 9/19 10:48）のままで、9/20・9/21の新版なし。`dist-tags`は`latest`・`next`とも`2.1.278`／`stable: 2.1.267`。レジストリの`modified`は`2026-09-19T03:10:16.949Z`。手元は`2.1.243`
- **【差分なし】CHANGELOG.md**（curl）：先頭は`## 2.1.278`（auto modeのclassifier課金・9/20に記録済み）。その上に新しい節なし
- **【差分なし】Claude Apps リリースノート**（curl）：最新は**September 15, 2026**（Salesforce in Claude・ベータ）のまま。**あとから足されたエントリも無し**（9/10 Smart reports・9/1 Fable 5.1／Mythos 5.1・8/25 Memoryまで既記録と一致）
- **【差分なし】Anthropic Newsroom**（curl・記事ページで日付を個別確認）：最新は**Sep 18**（`accenture-embedded-evaluation`＝9/19記録済み）。その次は**Sep 17**（`life-sciences-verification-program`）。一覧の先頭にある`improving-alignment-security-efforts`は**Aug 31**の記事で、新規ではない
- **【差分なし】claude.com/blog**（curl・記事ページで`datePublished`を個別確認）：最新は**9/17**（Balyasny事例）のまま。**一覧の上部9件は特集枠で、日付順ではない**——`claude-in-chrome-generally-available`（Aug 26）・`cowork-built-in-browser`（Aug 26）・`artifacts-in-claude-code`（**Jun 18**）・`claude-for-foundation-models`（Jun 08）・`claude-managed-agents-updates`（May 19）・`new-in-claude-managed-agents`（May 19）・`connectors-for-everyday-life`（Apr 23）は全部過去記事だった。**⚠️ 次回以降、この面は「一覧の並び順」で新旧を判定しない。記事ページの`datePublished`で確認する**
- **【差分なし】@ClaudeDevs**（実Chrome）：最新は**9/18**のProjects（`research-20260918-04`で記録済み・200万表示／1.1万いいね）
- **巡回チェック**：CHANGELOG ✅／npm ✅／Claude Apps ✅／Newsroom ✅／claude.com/blog ✅／@ClaudeDevs ✅
...
**本日の最大の収穫は1件で、しかも「一度閉じたネタが、閉じた理由ごと解消された」という形。****Claudeの出力に入る透かしの対象モデルが、公式ヘルプで名指しされた（Fable 5.1・Mythos 5.1）。**8/19に記録し8/28に閉じた`research-20260819-02`（透かし）は、**閉じたときの2つの⚠️（対象モデルが公式に出ていない／検出APIが未提供）が両方とも解消している。**

**フル版の役割どおり、デイリーが見ていない面を開いた。**本日の収穫は[Claude Platform リリースノートの2026-09-01エントリ](https://platform.claude.com/docs/en/release-notes/overview)の**下のほうにぶら下がっていた1項目**で、**9/2のデイリーは同じ日付エントリを開いてFable 5.1の公開だけを記録し、この項目を拾っていない。**8/28に②で確認した「1つの日付に複数の見出しがぶら下がる形式では、日付だけを見て閉じない」が、**①でも起きた。**

### Claude Code / Anthropic

- **【最大の収穫・採る】Claudeの透かしの「対象モデル」が公式に名指しされた。Fable 5.1 と Mythos 5.1**
  - **一次情報2点**（どちらも本日CCがブラウザで直接開いて確認）
    - [Claude Platform リリースノート 2026-09-01](https://platform.claude.com/docs/en/release-notes/overview)（verbatim）：`Text generated by Claude Fable 5.1 and Claude Mythos 5.1 carries Anthropic's text watermark, and supported image, video, and audio files that Claude produces through the code execution tool carry C2PA Content Credentials when you retrieve them through the Files API on the Claude API. Marking requires no changes to your requests or response handling.`
    - [公式ヘルプ「How Claude marks AI-generated content」](https://support.claude.com/en/articles/16266773-how-claude-marks-ai-generated-content)（verbatim）：`Claude models launched on or after August 2, 2026 support marking at launch. Models currently supported include Fable 5.1 and Mythos 5.1.` ／ `Claude markings cover output from supported models everywhere you use Claude, including Claude Platform (API), Claude, Claude Code, Claude Cowork, and Claude Tag.`
  - **8/28に閉じた理由が2つとも解消した**
    | 8/28に閉じたときの⚠️ | 本日の状態 |
    |---|---|
    | `supported models`の具体的なモデル名一覧は公式に出ていない | **解消。**Fable 5.1・Mythos 5.1と名指しされた |
    | 検出APIも検出ツールもまだ出ていない | **一部解消。**公式ヘルプ（verbatim）：`Watermark detection is currently in private preview, available to eligible organizations as required under EU law (such as regulators, law enforcement, media, fact-checkers, independent researchers, educational organizations, and EU civil society groups).` **＝存在はするが、個人は使えない** |
  - **使う側の何が変わるか（ここが本体）**：**どのモデルで書いたかで、透かしの有無が変わる。**Fable 5.1は9/2に記録したとおり**Claude Codeの既定Fableモデル**（[CHANGELOG `## 2.1.257`](https://raw.githubusercontent.com/anthropics/claude-code/main/CHANGELOG.md)・verbatim`Added Claude Fable 5.1 (claude-fable-5-1), now the default Fable model`）。**`/model`でFableを選べば透かしが入り、8/2より前に出たモデルを選べばまだ入っていない**（公式ヘルプは既存モデルを`in progress`と書いている）
  - ⚠️ **「Claudeの出力すべてに透かしが入る」と書かない。**入るのは**8/2以降にローンチしたモデル**で、現時点で名前が出ているのは**2つだけ**。既存モデルは移行期間の途中
  - ⚠️ **「AIで書いたことがバレるようになった」と書かない。**検出は**private preview**で、対象は規制当局・報道・研究機関などのEU法上の適格組織。**個人が自分の文章を検定する手段は今日時点で無い**
  - ⚠️ **8/14の公式ブログの記述は今日も生きている**（[How Claude's text watermarking works](https://www.anthropic.com/news/claude-text-watermark)・verbatim）：`Light editing probably won't remove the watermark completely; a complete rewrite where every word is replaced will.` ／ コードは`has generally less watermarking than some other forms of text`
  - ⚠️ **このMacで実機確認はできない。**透かしは不可視で、検出手段が個人に開放されていない。**「自分の記事に入っているか確かめた」は書けない**
  - **判定：X向き＝採る**（採用基準の「使える範囲」＋「既定モデル」が動いた）。**ただし`ideas.md`止まり**（下の在庫判定）
- **【差分・記録のみ・採らない】同じ9/1エントリの残り6項目**（[Claude Platform リリースノート](https://platform.claude.com/docs/en/release-notes/overview)）。**9/2のデイリーはこの日付エントリからFable 5.1の公開だけを記録していたので、残りを全部数えて記録する**
  1. `tool_choice`の`any`・`tool`が5.1モデルで非対応になり400を返す（`auto`・`none`は変更なし）
  2. thinkingブロックは、それを生成したモデルか、それより新しいモデルでしか読めない
  3. **Per-Message Effort（ベータ）**：会話の途中で`output_config.effort`を差し替えてもプロンプトキャッシュが保たれる
  4. **Turn-Scoped System Messages（ベータ）**：`clear_at: "next_user_message"`で、そのターンだけ効くシステムメッセージを置ける
  5. `thinking.display`に`"updates"`が増えた
  6. Fable 5.1・Mythos 5.1は**30日のデータ保持が必須**で、zero data retentionでは使えない
  - **判定：X向き＝採らない。**すべてAPI・SDKを書く人向けで、**このメディアの読者の手元では何も変わらない**
- **【差分・記録のみ・採らない】8/27付の`Personal and Service Account Keys`**（同リリースノート）。Claude Consoleで個人キー・サービスアカウントキーを作れるようになり、アカウントが組織から外れると失効する。**判定：採らない**（API運用の話）
- **【差分なし】Anthropic Newsroom**：[anthropic.com/news](https://www.anthropic.com/news)を本日ブラウザで直接確認。**最新は9/1「Developing Enterprise Frontier Safeguards with our customers」のままで、9/2〜9/4の新規は0件**（9/2に記録済み）
- **【差分なし】Claude Apps リリースノート**：[support.claude.com](https://support.claude.com/en/articles/12138966-release-notes)を本日確認。**9/1（Fable 5.1／Mythos 5.1）が最新のまま**
- **【本日は確認していない】@ClaudeDevs・CHANGELOG・npmの`time`**：本日8:05のデイリー回が3つとも開いており（`2.1.260`＝`next`タグ・CHANGELOGに項目なし／Function Hooksの提案）、**同日中の再取得は行っていない。**フル版の主目的は「デイリーが見ていない面を開くこと」（2026-08-21確定）
...
**①はフル版の担当チャネルを開いた結果、差分なし。**本日8:00のデイリー回が拾った週次上限の件（9/14から標準+25%・今と比べて17%減）が本日唯一の①の材料で、**フル版が開く3チャネル（Claude Platform リリースノート・Claude Apps リリースノート・Claude Academy）には新規エントリがなかった。**

### Claude Code / Anthropic

- **Claude Platform リリースノートは 2026-08-27 付が最新のまま**（[platform.claude.com](https://platform.claude.com/docs/en/release-notes/overview)を**本日CCが直接開いて全エントリを確認**）。8/28〜8/31の新規エントリなし
  - 8/27付の内容（8/28のフル版が記録済み・再掲しない）：各SDKで`client.beta.files`・`client.beta.skills`がベータヘッダを送らなくなり、`BetaSkill`が`BetaContainerSkill`へ改名
- **Claude Apps リリースノートは 2026-08-25 付が最新のまま**（8:00のデイリー回の確認と一致）
- **Claude Academy に新しい学習リソースの追加なし**（[academy.claude.com](https://academy.claude.com/)を本日確認）。掲載は`AI Fluency: Framework & Foundations`（14レッスン＋クイズ1・4時間）／`The 4 Properties of AI`（チュートリアル・7分）／`AI Capabilities and Limitations`（13レッスン＋クイズ1・3.5時間）
  - ⚠️ **サイトに公開日・更新日の表示がない。**8/24に「2026-08-20公開」と記録したのは二次情報（gihyo.jp）が根拠で、**サイト本体からは新旧を判定できない。**次回以降も「新着があったか」はサイトからは判断できないものとして扱う
  - **M-14（Claude Academyの記事ネタ）の着手条件は未達のまま**（実際にコースを受けること）。本日も変化なし
- **週次上限の件は本日8:00のデイリー回が記録済み**（`LIMIT-01`として下書きを書き直し済み）。フル版で重複して記録しない
