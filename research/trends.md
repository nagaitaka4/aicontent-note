...
- **Claude Fable 5 & Mythos 5 正式リリース（2026-06-09）**：Anthropicが最上位モデル2種を公開。Fable 5はClaude API・Amazon Bedrock・Vertex AI・Microsoft Foundryで一般提供。Mythos 5はProject Glasswing参加の承認顧客に限定提供。1Mトークンコンテキスト・最大出力128Kトークン・知識カットオフ2026年1月。価格は入力$10/M・出力$50/Mトークン。6月22日まではPro・Max・Team・Enterpriseプランで追加費用なし（Anthropic公式・TechCrunch, 2026-06-09）
- **米政府輸出規制指令でFable 5・Mythos 5のアクセス停止（2026-06-12）**：リリースから3日後、Anthropicが米政府の輸出規制指令を受領し両モデルへのアクセスを停止。安全分類器によるフォールバック（高リスク領域でOpus 4.8に切り替え）を実装済みだったが政府指令には対応できず。現在はOpus 4.8が実質最上位として稼働（CNBC, 2026-06-09/12）
- **Claude Code June 2026 アップデート**：ネストサブエージェント対応・プラグイン検索機能追加・Chrome/VSCode/ターミナル連携強化。WebおよびモバイルセッションがPR自動追跡・CI失敗自動修正・レビューコメント自動対応可能に。`/powerup` インタラクティブチュートリアル・`/team-onboarding` ガイドが追加。レート制限も倍増（Releasebot・builder.io, 2026-06）

### AIコンテンツ・業務効率化トレンド

- **コンテンツマーケティング実務者の約6割がAI検索の影響を実感・対策意向は8割**（日本SPセンター調査, 2026）。「AIへの対策」意識が急速に広まり、AI検索対応は差別化から「必須対応」フェーズへ移行している
- **「AIをただ使う」段階が終わり「使いこなす」フェーズへ**：効果測定自動化・改善提案AIなどツールが専門分化。少人数体制が完全定着し、安易な増員よりAI活用で成果を出すスタイルが標準化（CANVAS d2cr.co.jp, 2026）
- **AIエージェント・RAG構築が急増中のホットカテゴリ**（AIトレンドレポート2026・aismiley.co.jp）。「エージェントを設計・監督する」役割が業務上の新しい専門スキルとして浮上
